...
@HiltViewModel
class VermifucacaoViewModel @Inject constructor(
    ...
    private fun filterVermifugacaoNaoAplicadas(medicamentos: List<MedicamentoWithAplicacoes>): List<MedicamentoWithAplicacoes> {
        return medicamentos.filter { it.medicamento.tipo == MedicamentoTipo.VERMIFUGACAO }
            .filter { it.aplicacoes.isNotEmpty() && isVacinaNaoAplicada(it.aplicacoes[0]) }
    }

    private fun updateStateWithVermifugacaoNaoAplicadas(lista: List<MedicamentoWithAplicacoes>) {
        _state.update {
            it.copy(
    ...
    private fun recuperaVermifugacaoNaoAplicadas(petId: String) {
        viewModelScope.launch {
            repository.getMedicamentosWithAplicacaoByPetId(petId).collect { medicamentos ->
                val lista = filterVermifugacaoNaoAplicadas(medicamentos)
                ...
                updateStateWithVermifugacaoNaoAplicadas(sortedMedicamentos)
            }
        }
    }
    ...
    private fun recuperaMedicamentos(petId: String) {
        viewModelScope.launch {
            repository.getMedicamentosWithAplicacaoByPetId(petId)
                .map { allItens ->
                    allItens.filter {
                        it.medicamento.tipo == MedicamentoTipo.VERMIFUGACAO
                    }
                }
                .collect { itens ->
        ...
        when (event) {
            ...
            is VacinaEvent.OnSaveVacina -> {
                ...
                val medicamento = Medicamento(
                    ...
                    tipo = MedicamentoTipo.VERMIFUGACAO,
                    petId = petId
                )
